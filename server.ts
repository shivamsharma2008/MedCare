...
`;
    ...
    if (errMessage.includes("resource_exhausted") || errMessage.includes("quota") || errMessage.includes("rate-limit")) {
      return res.status(429).json({
        error: "Gemini AI API quota exceeded or rate limited. Please try again later or enter prescription details manually.",
        isQuotaExceeded: true,
      });
    }
    return res.status(500).json({
      error: errMessage || "Failed to analyze parcha with Gemini AI",
    });
  }
});
