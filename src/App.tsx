...
const AppContent: React.FC = () => {
  const { user, role, loading, currentRoute, navigateTo } = useAuth();

  // Role-based protection: if authenticated and path is mismatched, correct the route
  useEffect(() => {
    if (!loading && user) {
      if (role === "doctor" && currentRoute.startsWith("/patient")) {
        navigateTo("/doctor/home");
      } else if (role === "patient" && currentRoute.startsWith("/doctor")) {
        navigateTo("/patient/home");
      } else if (currentRoute === "/" || currentRoute === "/login" || currentRoute === "/register") {
        navigateTo(role === "doctor" ? "/doctor/home" : "/patient/home");
      }
    }
  }, [user, role, loading, currentRoute, navigateTo]);
  ...
  if (!user) {
    if (
      ...
      currentRoute.startsWith("/patient/register") ||
      currentRoute.startsWith("/doctor/register")
    ) {
      return (
        <div className="min-h-screen bg-slate-100 flex flex-col">
          <Navbar />
